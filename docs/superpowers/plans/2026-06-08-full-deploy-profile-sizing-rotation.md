...
- [ ] **Step 1: 진입점 확정 (조사 완료됨)** — Task 5에서 확정 사용:
  - 매도: `order_executor.execute_sell(symbol, qty, price=0(시장가), reason, name)` (order_executor.py:374)
  - 매수: `decision_engine._emit_signal(symbol, candidate, price, matched)` → 내부에서 trading_signals INSERT + `order_executor.execute_signal`(Task 2 Profile 사이징 적용) (decision_engine.py:1413)
  - 보유 qty/이름: `position_manager.get_positions()`

Run (재확인): `grep -nE "async def execute_sell|async def _emit_signal|async def execute_signal" backend/services/engine/order_executor.py backend/services/engine/decision_engine.py`
Expected: 세 메서드 존재 확인.
...
기존 `replacement_result = await evaluate_replacement_signals(...)` 직후(~1033)에 추가(`new_candidates`는 해당 스코프에 존재):
```python
