...
- PM 최종 정책은 `손절/트레일링 감시는 S8 관리 포지션 기준`, `관리자 지정 청산 시간에는 KIS 계좌 실보유 전체 전량 시장가 매도`이다. 이 시스템은 장기 보유 운용이 아니라 좋은 종목 선별과 당일 리스크 정리를 목표로 하므로, 시간청산 시 수동/장기/이월/봇 매수 여부를 구분하지 않는다.
...
0. 보유 포지션 자동 손절/트레일링 감시 결함 수정
   ...
   - 관리자 지정 청산 시간에는 KIS 계좌 실보유 전체를 수동/장기/이월/봇 매수 여부와 무관하게 전량 시장가 매도한다.
   - Trading Monitor는 `자동감시중 / 미감시 / 상태불일치`와 별도로 `시간청산 대상` 여부를 표시한다.
...
## 구현 범위
- [x] P0: 손절/트레일링 감시 대상과 시간청산 대상을 분리 정의
- [x] P0: 관리자 지정 청산 시간에는 KIS 실보유 전체를 일괄 시장가 매도하도록 S9/EOD 청산 경로 수정
- [x] P0: 보유 종목별 `자동감시중 / 미감시 / 상태불일치`와 `시간청산 대상` 표시 추가
- [x] P0: `position_stop_states`의 날짜/상태 기준 보강 또는 stale row 차단
- [x] P0: 트레일링 활성화/고점/active stop 상태가 DB와 화면에 일관되게 반영되도록 보강
- [x] 백엔드 감사 로그와 화면 상태 표시 연결 지점 확인
- [x] 상태 표시를 `데이터 없음 / 미수집·대기 / 실행 실패`로 구분
- [x] Trade History 데이터 범위를 전체 주문 이력으로 확장 또는 표시 기준 수정
- [x] Trade Review DB 원본 저장과 MD 백업 생성 경로 설계 및 구현
- [x] Settings 추천값 자동 반영/승인 필요 경계 적용
- [x] Knowledge 전략 상태값 `승인 / 보류 / 개발필요` 흐름 적용
- [x] Trading Monitor, Daily Plan, Funnel Monitor, System Diagnostics 화면 목적에 맞게 표시 정리
- [x] 관련 서버 로그를 시작/완료/실패 단계로 남김
- [x] 테스트 결과서와 사용자 매뉴얼/WBS 업데이트

## 구현 완료 기록 — 2026-05-10
- P0 포지션 감시/시간청산 항목은 `b2c1949`~`599476f` 및 관련 테스트 결과서로 반영했다.
- Knowledge/Settings 안전 경계는 `7644250`, `9729d1d`, `75b6240`으로 반영했다.
- Backend audit/empty-state, Trade History 전체 주문 이벤트 표시, Trade Review DB 원본 + MD 백업 확인 메타데이터는 본 후속 작업에서 반영했다.
- 검증 명령: `python -m py_compile backend/services/engine/review_audit.py backend/api/routes/review_audit.py backend/api/routes/orders.py`, `node --check` 대상 콘솔 JS, `python -m unittest tests.unit.test_expert_knowledge_safety tests.unit.test_position_monitoring`, `npx playwright test tests/e2e/status-truth.spec.cjs --reporter=list`, route function smoke for `/api/v1/orders/range` and `/api/v1/review-audit/today`.
...
- 시간청산 실행 전 최종 운영 확인 문구와 버튼 위치.
- 비거래일/장외 시간에 시간청산이 호출될 경우의 화면 메시지.
...
- [x] PM이 이 개발계획서와 테스트계획서를 승인했다.
- [x] 실제 활성 코드 경로를 확인한 뒤 구현했다.
- [x] 모든 신규/수정 함수와 주요 컴포넌트에 목적 주석이 있다.
- [x] 주요 서버 작업은 INFO/WARN/ERROR 로그로 시작/완료/실패를 확인할 수 있다.
- [x] 관련 API 호출 테스트가 통과했다.
- [x] 관련 E2E 테스트가 통과했다.
- [x] 빌드 또는 정적 검증이 통과했다.
- [x] 기존 주문 실행, Settings, History 조회 기능이 깨지지 않았다.
- [x] 테스트결과서, 사용자 매뉴얼, WBS가 업데이트됐다.
...
|---|---|---|---|
| 지정시간 계좌 전체 청산 | 단위/통합 테스트 | KIS 계좌 실보유 2종목과 비어 있는 `PositionManager`를 준비한다 | 관리자 지정 청산 경로가 KIS 실보유 전체를 전량 시장가 매도 대상으로 만든다 |
| -16% 보유종목 손절 감시 | API + 단위/통합 테스트 | KIS 실보유에는 있으나 `PositionManager` 인메모리에 없는 손실 포지션을 준비한다 | 손절/트레일링은 `미감시`로 표시하되 지정시간 청산 대상임을 별도 표시 |
...
### 브라우저 확인 항목
- [x] Console 주요 카드가 audit 기준 상태를 표시한다.
- [x] Trading Monitor가 각 보유 종목의 `자동감시중 / 미감시 / 상태불일치`와 `시간청산 대상`을 구분해서 보여준다.
- [x] Trading Monitor가 위험/포지션 감시 정보를 우선 보여준다.
- [x] Daily Plan 상단에서 오늘 전략 요약을 먼저 확인할 수 있다.
- [x] Funnel Monitor에서 탈락 수와 탈락 이유를 확인할 수 있다.
- [x] System Diagnostics가 감사 로그 확인 중심으로 보인다.
- [x] Knowledge에서 전략 상태를 `승인 / 보류 / 개발필요`로 구분할 수 있다.
- [x] Trade History에서 전체 주문 이력이 날짜별로 조회된다.
- [x] Trade Review 결과를 화면과 MD 백업으로 확인할 수 있다.

### 엣지 케이스
- KIS 실보유에는 있지만 `PositionManager` 인메모리에는 없는 종목이 손절/트레일링 미감시이면서 시간청산 대상인 경우.
