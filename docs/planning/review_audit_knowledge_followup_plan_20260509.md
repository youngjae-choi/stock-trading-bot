...
## 구현 범위
- [ ] P0: 손절/트레일링 감시 대상과 시간청산 대상을 분리 정의
- [ ] P0: 관리자 지정 청산 시간에는 KIS 실보유 전체를 일괄 시장가 매도하도록 S9/EOD 청산 경로 수정
- [ ] P0: 보유 종목별 `자동감시중 / 미감시 / 상태불일치`와 `시간청산 대상` 표시 추가
...
- [ ] Trading Monitor가 각 보유 종목의 `자동감시중 / 미감시 / 상태불일치`와 `시간청산 대상`을 구분해서 보여준다.
