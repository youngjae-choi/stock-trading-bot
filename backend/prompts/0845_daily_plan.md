...
거래대금은 선정 기준이 아니다 — 거래대금만을 근거로 후보를 제외하지 않는다.
후보는 등락률·거래량급증(모멘텀)으로 이미 선별됐으므로, 위 4개 조건(서킷·공시의혹·거래정지·false_positive)에
해당하지 않으면 거래량 기준으로 Risk Profile을 배정한다.
