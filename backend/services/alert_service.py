...
async def send_telegram_approval_message(text: str, approval_id: str) -> bool:
    """승인/거절 두 버튼이 포함된 텔레그램 메시지 발송."""
    if not telegram_enabled():
        return False
    ...
    payload: dict[str, Any] = {
        ...
        "reply_markup": {
            "inline_keyboard": [[
                {"text": "✅ 승인", "callback_data": f"approve_action_plan_{approval_id}"},
                {"text": "❌ 거절", "callback_data": f"reject_action_plan_{approval_id}"},
            ]]
        },
    }
    try:
