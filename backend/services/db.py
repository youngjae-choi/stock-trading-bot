...
def _seed_system_settings(connection: sqlite3.Connection) -> None:
    ...
    defaults = [
        ...
        (
            ...
            True,
            "boolean",
            "스케일아웃 수확 모드. ON이면 +목표 도달 시 부분확정+잔량 러너, OFF면 기존 탐색모드(익절 없음) 보존",
        ),
        (
            "engine.scaleout_target_rate",
            0.02,
            "number",
            "스케일아웃 확정 매도 목표 수익률 (0.02 = +2%). PM 목표 '하루 +2% 확정'의 발동선",
        ),
        (
            "engine.scaleout_ratio",
            0.6,
            "number",
        ...
        # ── Phase 2: 진입 선별성 ──
        (
            "engine.calibration_gate_enabled",
            True,
            "boolean",
            "칼리브레이션 게이트. 누적 실적(표본 충분)이 나쁜 confidence 구간의 신규 진입 차단",
        ),
        (
            "engine.calibration_gate_min_samples",
            30,
            "number",
            "칼리브레이션 게이트 발동 최소 누적 표본 수 (bin당)",
        ),
        (
            "engine.calibration_gate_gap",
            0.15,
            "number",
            "칼리브레이션 게이트 승률 갭 임계값 (기대-실제 ≥ 이 값이면 차단)",
        ),
        (
            "engine.entry_fail_cooldown_enabled",
            True,
            "boolean",
            "entry_fail 쿨다운. 최근 반복 손실 진입 심볼의 신규 진입 차단",
        ),
        ("engine.entry_fail_cooldown_days", 3, "number", "entry_fail 쿨다운 조회 기간(일)"),
        ("engine.entry_fail_cooldown_min_count", 2, "number", "entry_fail 쿨다운 발동 최소 반복 횟수"),
        (
            "engine.confidence_gate_mode",
            "regime_defensive",
            "string",
            "confidence 게이트 강제 모드: off(관찰만) | regime_defensive(risk_off/volatile만 강제) | always",
        ),
        (
            "engine.regime_confidence_bonus_risk_off",
            0.05,
            "number",
            "risk_off 레짐에서 confidence 하한 가산치",
        ),
        (
            "engine.regime_confidence_bonus_volatile",
            0.10,
            "number",
            "volatile 레짐에서 confidence 하한 가산치",
        ),
        # ── Phase 3: 레짐 심화 + 폭락장 방어 ──
        (
            "engine.enforce_new_entry_allowed",
            True,
            "boolean",
            "레짐 SET의 new_entry_allowed=False를 실제 진입 차단으로 강제 (기존 표시용→강제 전환, OFF면 구동작)",
        ),
        (
            "risk.flash_crash_defense_enabled",
            True,
            "boolean",
            "장중 급락(flash crash) 방어 모드. KOSPI 급락 감지 시 디바운스 우회 레짐 전환 + 신규진입 차단",
        ),
        (
            "risk.flash_crash_threshold_pct",
            -2.0,
            "number",
            "급락 감지 KOSPI 등락률 임계값(%). 이하로 떨어지면 방어 모드 발동",
        ),
        (
            "engine.crash_playbook_enabled",
            True,
            "boolean",
            "폭락장 플레이북. 방어 모드에서 인버스 1x ETF만 예외 진입 허용해 하락장 수익 경로 확보 (2X/레버리지 차단 유지)",
        ),
        (
            "engine.crash_playbook_budget_rate",
            0.30,
            "number",
            "폭락장 플레이북 인버스 포지션 예산 상한 (계좌 잔고 대비 비율)",
        ),
